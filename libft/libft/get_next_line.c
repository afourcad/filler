...
/*   Updated: 2017/08/21 16:19:33 by afourcad         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*ft_get_alloc_line(t_buff *buff, char *line)
{
	...
	if ((tmp = (char *)malloc(sizeof(*tmp) * (size + 1))) == NULL)
		return (NULL);
	if (line != NULL)
		tmp = ft_beg_memcpy(tmp, line, buff->size_line);
	tmp = ft_beg_memadd(tmp, buff->buff, buff->size_line, buff->eol);
	tmp[size] = '\0';
	...
	while (i < buff->ret)
	{
		if (buff->buff[i] == EOL)
		{
			...
			*line = ft_get_alloc_line(buff, *line);
			...
			ft_beg_memmove(buff->buff, buff->buff + i + 1, buff->ret);
			buff->size_line = 0;
			return (1);
		}
		i++;
